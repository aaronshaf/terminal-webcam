...
class WebcamViewer {
  ...
  init() {
    ...
    console.log('M: Change display mode');
    console.log('Q: Quit\n');
    ...
    setTimeout(() => {
      ...
      // Setup keyboard - IMPORTANT: Set up the listener properly
      this.setupKeyboard()
    }, 1000)
  }
  ...
  private initTextGrid() {
    ...
    const help = new TextRenderable("help", {
      content: `[F]ilter [M]ode [Q]uit`,
      x: Math.floor(this.width / 2) - 11,
      y: this.height - 1,
      fg: "#888888",
      bg: "#222222",
      zIndex: 10,
  ...
  private setupKeyboard() {
    // Debug log
    console.error('Setting up keyboard listener...')
    
    this.renderer.on("key", (key: Buffer) => {
      const keyStr = key.toString()
      console.error(`Key pressed: ${keyStr.charCodeAt(0)} - "${keyStr}"`) // Debug
      
      // Handle both lowercase and uppercase
      switch(keyStr.toLowerCase()) {
        case 'f':
          console.error('Next filter')
          this.nextFilter()
          break
        case 'd':
          console.error('Previous filter')
          this.prevFilter()
          break
        case 'm':
          console.error('Cycle mode')
        ...
        case 'q':
          ...
          break
        default:
          console.error(`Unknown key: ${keyStr}`)
      }
    })
  }
...
async function main() {
  const renderer = await createCliRenderer({
    exitOnCtrlC: false,  // Changed to false so we handle keys ourselves
    stdin: process.stdin,
